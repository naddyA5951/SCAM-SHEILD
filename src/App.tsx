...
export default function App() {
  const [currentView, setCurrentView] = useState<"scanner" | "login">("scanner");
  ...
  // Check browser URL path on startup so /login directly opens LoginPage
  useEffect(() => {
    const syncViewFromLocation = () => {
      const path = window.location.pathname.toLowerCase();
      const hash = window.location.hash.toLowerCase();
      if (path === "/login" || hash === "#login" || window.location.search.includes("view=login")) {
  ...
  const navigateToView = (view: "scanner" | "login") => {
    setCurrentView(view);
  ...
  return (
    <div className="min-h-screen bg-transparent text-slate-100 flex flex-col font-sans selection:bg-cyan-500 selection:text-slate-950 relative overflow-x-hidden">
      {/* Hand-coded interactive 3D Cyber Background */}
      <CyberBackground3D />
      ...
      <Header
        ...
        {...({
          ...
          onNavigateScanner: () => navigateToView("scanner"),
      ...
      <main className="relative z-10 flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-10">
        {currentView === "login" ? (
          <LoginPage
            ...
            onBackToHome={() => navigateToView("scanner")}
          />
        ) : (
          <>
            {/* Hero Section */}
            <section className="text-center max-w-3xl mx-auto space-y-4 pt-2">
              <div className="flex flex-wrap items-center justify-center gap-2">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-950/70 border border-cyan-500/40 text-cyan-300 text-xs font-mono shadow-sm">
                ...
                {!user && (
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-cyan-500/20 hover:bg-cyan-500/30 border border-cyan-400/50 text-cyan-200 text-xs font-mono font-semibold transition-all hover:scale-105"
                  >
                    <span>⚡ Go to Login Page</span>
                    <span className="text-[10px] bg-cyan-400 text-slate-950 font-bold px-1.5 py-0.2 rounded">3D Tilt</span>
                  </button>
                )}
              </div>
